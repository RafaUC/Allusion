...
import { getCachedValue, makeCacheEntry, DbLookupCacheEntry } from './indexer';
...
class LocationStore {
  ...
  private readonly dbFileByPathCache = new Map<string, DbLookupCacheEntry>();
  private readonly dbFileByInoCache = new Map<FileDTO['ino'], DbLookupCacheEntry>();
  ...
  private getCachedDbFileByPath(path: string): FileDTO | undefined | null {
    return getCachedValue(this.dbFileByPathCache, path);
  }

  private getCachedDbFileByIno(ino: FileDTO['ino']): FileDTO | undefined | null {
    return getCachedValue(this.dbFileByInoCache, ino);
  }

  private cacheDbLookup(path: string, ino: FileDTO['ino'], file: FileDTO | undefined): void {
    const entry = makeCacheEntry(file, this.dbLookupCacheTtlMs);
    this.dbFileByPathCache.set(path, entry);
    this.dbFileByInoCache.set(ino, entry);
  }
  ...
  @action async changeLocationPath(location: ClientLocation, newPath: string): Promise<void> {
    ...
    if (shouldRestartWatcher(newLocation, previousPath)) {
